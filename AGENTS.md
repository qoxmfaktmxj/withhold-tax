...
⚠️ **현재 상태**: Next.js 앱이 존재하며 포팅 산출물이 구성되어 있음(이벤트 기반 문서/콘텐츠 동기화는 계속 진행 중).
콘텐츠·법령 변경 작업은 먼저 근거 문서와 `content/facts.json`/MDX 연결 구조를 확인한 뒤, 요청 범위 안에서 코드·데이터를 수정할 것.
...
DESIGN-AGENTS.md ──► Anthropic Codex.com 디자인 시스템 (포팅 시 목표 디자인)
```
...
- **DESIGN-AGENTS.md** — 원본 HTML과 **다른** 디자인 시스템. 포팅 시 적용할 목표 디자인.
...
**한글 본문이 핵심**이므로 DESIGN-AGENTS.md의 영문 폰트를 그대로 쓰지 말 것:
