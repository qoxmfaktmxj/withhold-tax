...
describe('tool navigation', () => {
  ...
  it('shows reference health as status badges on the home page', () => {
    ...
    expect(within(status).getByRole('listitem', { name: '1차 원문 확인 140' })).toBeInTheDocument()
    expect(within(status).getByRole('listitem', { name: '원문 미확인 119' })).toBeInTheDocument()
    ...
    expect(within(status).getByRole('listitem', { name: '마지막 법령 점검 2026-06-15' })).toBeInTheDocument()
  })
