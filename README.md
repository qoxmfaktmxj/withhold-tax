...
<div align="center">
  ...
  <br/>
  <sub><strong>전체 본문 검색</strong> — 어느 페이지서든 <code>⌘K</code>(또는 <code>Ctrl+K</code>)로 12개 장 전체를 검색하고, 결과를 누르면 <strong>해당 장의 정확한 섹션으로 바로 이동</strong>합니다.</sub>
</div>
...
### 전체 본문 검색 (⌘K) — 어디서든, 정확한 위치로
모든 장(12장 + 부록)의 본문을 섹션 단위로 색인합니다. 어느 페이지에서든 `⌘K`/`Ctrl+K` 또는 사이드바 "전체 검색"으로 명령 팔레트를 열어 검색하고, ↑↓·Enter로 이동하면 **그 장의 해당 섹션 앵커로 바로 스크롤**됩니다. 빌드타임 인덱스(MiniSearch) + `rehype-slug` 앵커로 결과와 본문 위치가 정확히 일치합니다.
